...
final class MessageGateway extends BaseGateway
{
    ...
    public function getConversationMessages(int $conversation_id, int $limit = 20, ?int $olderThanId = null, int $offset = 0): array
    {
        ...
        foreach ($messages as $m) {
            $message = new Message(
                $m['is_htmlentity_encoded'] ? html_entity_decode($this->sanitizer->purifyHtml($m['body'])) : $m['body'],
                $m['foodsaver_id'],
    ...
    {
        return $this->db->update('fs_conversation', ['name' => $name ? strip_tags($name) : null], ['id' => $cid]);
    }
