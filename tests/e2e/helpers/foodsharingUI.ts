...
export class FoodsharingUI {
   ...
   */
  async addInTagSelect(value: string, tagSelectSelector: string = '#tags-with-dropdown'): Promise<void> {
    ...
    await this.page.click(`//button[contains(@class, 'dropdown-item') and contains(text(), '${value}')]`);
   ...
   * @param tagSelectSelector The CSS selector for the TagSelect component
   */
  async removeFromTagSelect(value: string, tagSelectSelector: string | null = null): Promise<void> {
    const tagXPath = tagSelectSelector + 
      `/descendant::li[contains(@class, "list-inline-item")]/*[contains(@title, "${value}")]`;

    const removeButtonXPath = tagXPath + 
      '/following-sibling::button[contains(@class, "b-form-tag-remove")]';

    await this.page.click(removeButtonXPath);
    
    await this.page.locator(tagSelectSelector + ' .list-inline-item')
