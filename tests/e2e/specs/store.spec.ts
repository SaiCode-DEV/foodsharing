...
test.describe('Store', () => {
	...
	let foodsaverNoRegion: Awaited<ReturnType<typeof foodsharing.createFoodsaver>>;
	let foodsaverOnJumperList: Awaited<ReturnType<typeof foodsharing.createFoodsaver>>;
	let foodsaverDifferentRegion: Awaited<ReturnType<typeof foodsharing.createFoodsaver>>;
	let storeManager: Awaited<ReturnType<typeof foodsharing.createStoreCoordinator>>;
	let foodsaverWithStoreManagerQuiz: Awaited<ReturnType<typeof foodsharing.createStoreCoordinator>>;
	...
	test.beforeEach(async () => {
		...
		foodsaverOnJumperList = await foodsharing.createFoodsaver(null, extraParams);
		storeManager = await foodsharing.createStoreCoordinator(null, extraParams);
		foodsaverWithStoreManagerQuiz = await foodsharing.createStoreCoordinator(null, extraParams);

		store = await foodsharing.createStore(
			region.id, null, null, { betrieb_status_id: 1 }
		);
		...
		await foodsharing.addRegionMember(region.id, foodsaverWithStoreManagerQuiz.id);

		await foodsharing.addStoreTeam(store.id, storeManager.id, true);
		await foodsharing.addStoreTeam(store.id, foodsaver.id);
		await foodsharing.addStoreTeam(store.id, foodsaverOnJumperList.id, false, true, true);

		// Edge case: user from other region
		const differentRegion = (await foodsharing.createRegion()).id;
		foodsaverDifferentRegion = await foodsharing.createFoodsaver(null, { bezirk_id: differentRegion });
		await foodsharing.addRegionMember(differentRegion, foodsaverDifferentRegion.id);
	});

	test('StoreManager and Foodsaver can see store on dashboard', async ({ page, acceptanceHelper }) => {
		// StoreManager
		await acceptanceHelper.login(storeManager.email);
		await expect(Database.seeInDatabase('fs_betrieb_team', {
			betrieb_id: store.id,
			foodsaver_id: storeManager.id,
			active: 1,
			verantwortlich: 1,
		})).resolves.toBeTruthy();
		await expect(page.getByRole('heading', { name: 'Deine Betriebsverantwortungen' })).toBeVisible();
		...
		await expect(page.getByRole('heading', { name: 'Deine Betriebe' })).toBeVisible();
		await expect(page.getByRole('link', { name: store.name })).toBeVisible();
	});
	...
	test.describe('Store – Access Restriction', () => {
		const users = [
			...
			{ label: 'FoodsaverNoRegion', getUser: (object: any) => object.foodsaverNoRegion },
			{ label: 'foodsaverDifferentRegion', getUser: (object: any) => object.foodsaverDifferentRegion },
		];

		for (const { label, getUser } of users) {
			test(`User ${label} cannot access store page`, async ({ page, acceptanceHelper }) => {
				const object = { foodsharer, foodsaverNoRegion, foodsaverDifferentRegion };
	...
	test('StoreManager can see pickup history', async ({ page, acceptanceHelper }) => {
		...
		while (await page.getByRole('button', { name: 'Vorheriger Monat' }).isEnabled()) {
			await page.getByRole('button', { name: 'Vorheriger Monat' }).click();
		}

		// Find the first active day (button, not aria-disabled="true")
		const activeDay = page.locator('.b-calendar-grid-body [role="button"]:not([aria-disabled="true"])').first();
		...
		await expect(page.getByText(`${foodsaver.name} ${foodsaver.nachname}`)).toBeVisible();
	});
	...
	test.describe('Store – can access store chat', () => {
		const roles = [
			...
			{ label: 'FoodsaverOnJumperList', userGetter: () => foodsaverOnJumperList },
		];

		for (const { label, userGetter } of roles) {
			test(`User ${label} can access store chat`, async ({ acceptanceHelper }) => {
				const user = userGetter();
				await acceptanceHelper.login(user.email);

				if (label === 'StoreManager') {
					await expect(Database.seeInDatabase('fs_foodsaver_has_conversation', {
	...
	test.describe('Store – remove member from store', () => {
		test('StoreManager can remove member', async ({ page, acceptanceHelper }) => {
			await acceptanceHelper.login(storeManager.email);
			await page.goto(`/store/${store.id}`);
			...
			await acceptanceHelper.waitForActiveAPICalls();
			await expect(page.locator('.store-team')).not.toContainText(`${foodsaverOnJumperList.name} ${foodsaverOnJumperList.nachname}`);
			await expect(Database.seeInDatabase('fs_betrieb_team', {
				betrieb_id: store.id,
				foodsaver_id: foodsaverOnJumperList.id,
			})).resolves.toBeFalsy();
		...
		test('Foodsaver cannot remove member', async ({ page, acceptanceHelper }) => {
			await acceptanceHelper.login(foodsaver.email);
			await page.goto(`/store/${store.id}`);
			await acceptanceHelper.waitForActiveAPICalls();
			...
			await expect(page.locator('.dropdown-menu.show >> text=Aus dem Team entfernen')).toHaveCount(0);

			await acceptanceHelper.logMeOut();
		});
	});

	test('StoreManager can promote and demote a member', async ({ page, acceptanceHelper }) => {
		...
		await expect(page.locator(`#user-${foodsaverWithStoreManagerQuiz.id}.manager`)).toBeVisible();
		...
		await expect(page.locator(`#user-${foodsaverWithStoreManagerQuiz.id}.manager`)).toHaveCount(0);
	...
	test('can invite store member', async ({ page, acceptanceHelper }) => {
		...
		await page.fill('#new-member-search input', foodsaverWithStoreManagerQuiz.name);
		await acceptanceHelper.waitForActiveAPICalls();
	...
	test('StoreManager can move member to jumper', async ({ page, acceptanceHelper }) => {
		// ensure member exists in team
		await foodsharing.addStoreTeam(store.id, foodsaver.id);
	...
	test('StoreManager can access store log', async ({ page, acceptanceHelper }) => {
		...
		await page.click('#search-store-log');
		await acceptanceHelper.waitForActiveAPICalls();
		await expect(page.getByText('auf die Springerliste gesetzt')).toBeVisible();
