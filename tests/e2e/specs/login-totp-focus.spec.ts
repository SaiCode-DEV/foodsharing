...
test.describe('Login TOTP focus', () => {
  test('reveals TOTP input and focuses it after 403 login response', async ({ page, acceptanceHelper }) => {
    ...
    await page.waitForSelector('#testing-login-input-totp', { state: 'visible', timeout: 5000 })

    // Wait until its inner input becomes the activeElement (focused)
    await page.waitForFunction(() => {
      const totpInput = document.querySelector('#testing-login-input-totp input') as HTMLElement | null
