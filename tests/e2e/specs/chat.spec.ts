...
test.describe('Chat', () => {
  ...
  test('can send and receive chat messages', async ({ page, acceptanceHelper }) => {
    ...
    await page.waitForSelector('#roomTextarea', { state: 'visible', timeout: 50000 });
    ...
    await expect(page.locator('.vac-message-card.vac-message-current')).toContainText(chatTextForFoodsaver1, { timeout: 20000 });
    expect(await Database.seeInDatabase('fs_msg', {
    ...
    const conversationsButton = page.getByRole('button', { name: /Nachrichten/i });
    await expect(conversationsButton.locator('.badge')).toContainText('1', { timeout: 10000 });

    // Open the conversation menu and open the new conversation
    await conversationsButton.click();
    await page.waitForSelector('.dropdown-menu.show .dropdown-item', { state: 'visible', timeout: 4000 });
    await page.locator('.dropdown-menu.show .dropdown-item').first().click();
    await page.waitForSelector('.vac-container, #roomTextarea', { state: 'visible', timeout: 6000 });
    ...
    await expect(page.locator('.vac-message-card.vac-message-current')).toContainText(chatTextForFoodsaver2, { timeout: 10000 });

    // Check that foodsaver1 receives the reply in the chatbox
    await expect(page.locator('.vac-message-card').first()).toContainText(chatTextForFoodsaver1, { timeout: 15000 });
