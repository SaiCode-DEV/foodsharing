...
test.describe('Login', () => {
  ...
  test('can login', async ({ page, acceptanceHelper }) => {
    ...
    await expect(page.locator('.testing-intro-field')).toContainText(`Hallo ${foodsharer.name}`);
    ...
    expect(sessionCookie?.expires).toBeGreaterThan(now + oneDayInSeconds - toleranceInSeconds);
    expect(sessionCookie?.expires).toBeLessThan(now + oneDayInSeconds + toleranceInSeconds);
  });
  ...
  test('can remember login', async ({ page, acceptanceHelper }) => {
    ...
    const toleranceInSeconds = 60 * 60; // 1 hour tolerance
    expect(sessionCookie?.expires).toBeGreaterThan(now + thirtyDaysInSeconds - toleranceInSeconds);
    expect(sessionCookie?.expires).toBeLessThan(now + thirtyDaysInSeconds + toleranceInSeconds);
