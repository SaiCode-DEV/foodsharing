...
test.describe("Food Basket", () => {
  ...
  test("can create, update, request and close a food basket", async ({
    ...
    await page.goto("/");
    await expect(page.locator(".testing-basket-dropdown")).toContainText(
      "Essenskörbe",
    );
    ...
    await page.waitForSelector(
      "text=Beschreibung, Bild, Übergabeort und Zeitraum sind öffentlich sichtbar.",
    );
    ...
    await expect(page.locator("body")).toContainText(
      "Bitte wähle eine Möglichkeit aus, wie der Essenskorb angefragt werden soll.",
    );
    ...
    await page.click(".testing-basket-dropdown > .nav-link");
    await expect(page.locator(".testing-basket-create")).toContainText(
      "Essenskorb anlegen",
    );
    ...
    await expect(page.locator("vue-advanced-chat")).toContainText(
      "Hi friend, can I have",
    );
    ...
    await page.click(".testing-basket-dropdown > .nav-link");
    await page.waitForSelector(".testing-basket-requests-close", {
      state: "visible",
    });
    await page.click(".testing-basket-requests-close");
    await page.waitForSelector(
      `text=Essenskorbanfrage von ${picker.name} abschließen`,
    );
    await expect(page.locator("body")).toContainText("Hat alles gut geklappt?");

    // Select "Nicht abgeholt" option (value="2")
    await page.click(
      '//label[@for=(//input[@name="basket-request-status"][@value="2"]/@id)]',
    );
    await expect(
