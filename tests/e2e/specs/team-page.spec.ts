...
test.describe('Team Page', () => {
  ...
  test.beforeEach(async () => {
    ...
    await foodsharing.addRegionMember(RegionIDs.TEAM_BOARD_MEMBER, boardMember.id);

    administrationMember = await foodsharing.createFoodsaver();
    await foodsharing.addRegionMember(RegionIDs.TEAM_ADMINISTRATION_MEMBER, administrationMember.id);

    alumniMember = await foodsharing.createFoodsaver();
    await foodsharing.addRegionMember(RegionIDs.TEAM_ALUMNI_MEMBER, alumniMember.id);
  });
