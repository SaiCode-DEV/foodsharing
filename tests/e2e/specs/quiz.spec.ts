...
test.describe("Quiz", () => {
  ...
  test.beforeEach(async () => {
    ...
    foodsharer = await foodsharing.createFoodsharer(null, {
      skip_quiz_creation: true,
    });
    foodsaver = await foodsharing.createFoodsaver(null, {
      skip_quiz_creation: true,
    });
  ...
  testCases.forEach(
    ({ userType, quizName, buttonTextPattern, description }) => {
      test(`can start quiz: ${description}`, async ({
        page,
        acceptanceHelper,
      }) => {
        ...
        await expect(
          page.locator("text=Jetzt das Quiz durchführen!").first(),
        ).toBeVisible();
        await page.getByRole("button", { name: buttonTextPattern }).click();
        await expect(
          page.getByRole("button", { name: "Los geht’s!" }),
        ).toBeVisible();
        ...
        await page.click(`text=${quizName}`);
        await expect(
          page.locator("text=Quiz jetzt weiter beantworten!"),
        ).toBeVisible();
        await page
          .getByRole("button", { name: "Quiz jetzt weiter beantworten!" })
          .click();
        await expect(
          page.getByRole("button", { name: "Los geht’s!" }),
        ).toBeVisible();
