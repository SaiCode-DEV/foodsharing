...
test('Foodsaver can join another region', async ({ page, acceptanceHelper }) => {
  ...
  const welcomeGroup = await foodsharing.createWorkingGroup('Begrüßung', { parent_id: region.id });
  ...
  await expect(page.locator('.testing-region-join-select')).toContainText('Bitte auswählen');
  await page.locator('.testing-region-join-select').selectOption({ label: region.name });
  await page.click('.testing-region-join .btn.btn-primary');
  await acceptanceHelper.waitForActiveAPICalls();
  await expect(page.locator('.regionTopClass')).toBeVisible();

  expect(await Database.seeInDatabase('fs_foodsaver_has_bezirk', { foodsaver_id: foodSaver.id, bezirk_id: region.id })).toBe(true);
  expect(await Database.seeInDatabase('fs_foodsaver_has_bell', { foodsaver_id: welcomeAdmin.id })).toBe(true);
  expect(await Database.seeInDatabase('fs_foodsaver_has_bell', { foodsaver_id: ambassador.id })).toBe(false);
});
