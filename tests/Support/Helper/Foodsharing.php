...
class Foodsharing extends Db
{
    ...
    public function createQuiz(int $quizId, ?int $questionCount = null): array
    {
        ...
        } elseif ($quizId === QuizID::FOODSHARER->value) {
            $params['name'] = 'Quiz für Foodsharer';
            $params['desc'] = 'Werde Foodsharer mit diesem Quiz!';
    ...
    {
        // Assemble a store name
        $city = $this->faker->city();
        $street = $this->faker->streetName();
        $name = $this->faker->company() . ' ';
        $name .= match (random_int(0, 2)) {
            0 => $this->faker->companySuffix(),
            1 => $city,
            2 => $street,
        };
        ...
        $params = array_merge([
            ...
            'plz' => $this->faker->postcode(),
            'stadt' => $city,
            'str' => $street . ' ' . $this->faker->buildingNumber(),
            'lat' => $this->faker->latitude(55, 46),
            'lon' => $this->faker->longitude(4, 16),
            'name' => $name,
    ...
    public function createWorkingGroup($name, $extra_params = [], bool $fillMailbox = true)
    {
        ...
        $includeThread = isset($extra_params['include_thread']) ? $extra_params['include_thread'] : false;
        if ($includeThread) {
            unset($extra_params['include_thread']);
        }

        $region = $this->createRegion($name, $extra_params, fillMailbox: $fillMailbox);
        if ($includeThread) {
            $this->addForumThread($region['id'], $includeThread, false, ['time' => $this->faker->dateTimeBetween('-8 months', 'now')]);
        }
    ...
    public function addGroupCategory(string $name, int $id): int
    {
        return $this->haveInDatabase('fs_group_category', ['name' => $name, 'id' => $id]);
    }
